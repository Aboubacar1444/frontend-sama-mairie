...
import { formatAmount, formatBusinessCount, formatCategoryName } from "@/pages/taxes/components/taxes-utils";

type TaxesListTableProps = {
    taxeList?: Taxe[],
    isLoading?: boolean,
    onDetailsClick: (taxe: Taxe) => void,
            ...
            <Table className="w-full min-w-[980px] border-spacing-0.1 border-separate border-neutral-300 dark:border-slate-600">
                <TableHeader>
                    <TableRow className="bg-neutral-200 dark:bg-slate-700 text-center">
                        ...
                        <TableHead className="px-4 h-12 text-center bg-neutral-100 dark:bg-slate-700 border-t border-neutral-200 first:border-s last:border-e dark:border-slate-600">Businesses</TableHead>
                ...
                <TableBody>
                    {isLoading ? (
                        <TableRow>
                            <TableCell colSpan={7} className="py-8 px-4 border-b text-center first:border-s last:border-e border-neutral-200 dark:border-slate-600">
                        ...
                        taxeList.map((taxe, index) => {
                            ...
                            return (
                                <TableRow key={taxe.id}>
                                    ...
                                    <TaxesCell isLast={isLast}>
                                        <div className="flex flex-col text-center md:text-left">
                                            <span className="font-medium text-neutral-900 dark:text-white">{taxe.title}</span>
                                            <span className="text-sm text-neutral-500 dark:text-neutral-300">{taxe.currency}</span>
                                        </div>
                                    </TaxesCell>
                                    ...
                                    <TaxesCell isLast={isLast} className="text-center">{formatBusinessCount(taxe.businesses.length)}</TaxesCell>
                                    <TaxesCell isLast={isLast} className="text-center">
